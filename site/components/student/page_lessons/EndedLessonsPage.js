...
function EndedLesson(props) {
    ...
    useEffect(() => {
        backendFetchGET('/getStudentSessionHistoryById?lessonId=' + props.lessonInfo.lesson_id, async (response) => {
...
export default function EndedLessonsPage(props) {
    ...
    useEffect(() => {
        backendFetchGET('/getStudentEndedLessons', async (response) => {
            if (response.status == 200) {
    ...
    const endedLessonElems = endedLessons.map(elem => {
        return <EndedLesson key={elem.lesson_id} lessonInfo={elem} />
    })
